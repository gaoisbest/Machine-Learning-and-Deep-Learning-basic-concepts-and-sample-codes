...
- Three approaches for image classification
  - k-nearest neighbor classifier, based on the pixel distance (L1 or L2). `k` and distance measure (L1 or L2) are determined by cross validation. The drawbacks of  algorithm are (1) storing all training set and (2) expensive predicting. Since totally different pictures may have the same distance, the performance of knn is not good.
  - Linear classifier.
